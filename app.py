...
alcohol = st.number_input("Alcohol Units per Week", min_value=0.0, value=0.0)
...
git = st.selectbox("Gastrointestinal Side Effects Observed?", ["No", "Yes"])
git = 1 if git == "Yes" else 0

genito = st.selectbox("Genitourinary Side Effects Observed?", ["No", "Yes"])
...
if st.button("🔍 Predict Risk"):
    ...
    if prob < 0.3:
        st.success("✅ Low Risk of Neurotoxicity - standard monitoring sufficient.")
    elif prob < 0.7:
        st.warning("⚠ Moderate Risk of Neurotoxicity - monitor patient closely.")
    else:
        st.error("🚨 High Risk of Neurotoxicity - consider closer monitoring and evaluation.")
